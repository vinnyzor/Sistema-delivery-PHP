...
function writeToLog($message, $restauranteId = null, $tipoAlteracao = null, $valorAntigo = null, $valorNovo = null) {
  ...
  if ($restauranteId !== null) {
    $logMessage .= " | Restaurante ID: {$restauranteId}";
  }
...
// Verifica se há uma mensagem de sucesso na URL
if (isset($_GET['msg_restaurante']) && $_GET['msg_restaurante'] == 1) {
  $mensagem = "<div class='alert alert-success alert-dismissible' role='alert'>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['imagem'])) {
  ...
  if ($imagem['size'] > $maxFileSize) {
    ...
    if (in_array($imagem['type'], $allowedTypes)) {
      ...
      $sql = "SELECT foto_perfil FROM restaurantes WHERE id='$restauranteId'";
      $result = $conn->query($sql);
      $row = $result->fetch_assoc();
      $valorAntigoImagem = $row['foto_perfil'];

      // Move a imagem para o diretório de upload
      if (move_uploaded_file($imagem['tmp_name'], $destino)) {
        // Atualiza o caminho da imagem no banco de dados
        $sql = "UPDATE restaurantes SET foto_perfil='$destino' WHERE id='$restauranteId'";
        if ($conn->query($sql) === TRUE) {
          // Salva a mensagem no log
          writeToLog("Imagem de perfil atualizada com sucesso para o usuário: {$email}", $restauranteId, "Imagem de Perfil", $valorAntigoImagem, $destino);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nome_estabelecimento'])) {
  $novoNomeEstabelecimento = $_POST['nome_estabelecimento'];
  $novoTelefone = $_POST['telefone'];

  // Obtém os valores antigos das informações do usuário
  $sql = "SELECT nome_estabelecimento, telefone, email FROM restaurantes WHERE id='$restauranteId'";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();
  $valorAntigoNome = $row['nome_estabelecimento'];
  $valorAntigoTelefone = $row['telefone'];
  $valorAntigoEmail = $row['email'];

  // Atualizar as informações do usuário no banco de dados
  $sql = "UPDATE restaurantes SET nome_estabelecimento='$novoNomeEstabelecimento', telefone='$novoTelefone' WHERE id='$restauranteId'";
  if ($conn->query($sql) === TRUE) {
    $mensagem = "<div class='alert alert-success alert-dismissible' role='alert'>
                  Informações do restaurante atualizadas com sucesso!
                  <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
    ...
    writeToLog("Nome Atualizado - ", $restauranteId, "Dados:", "Nome Antigo: {$valorAntigoNome}", "Novo Nome: {$novoNomeEstabelecimento}");
    writeToLog("Telefone Atualizado - ", $restauranteId, "Dados:", "Telefone Antigo: {$valorAntigoTelefone}", "Novo Telefone: {$novoTelefone}");
    header("Location: configuracoes.php?msg_restaurante=1");
  } else {
    $mensagem = "<div class='alert alert-danger alert-dismissible' role='alert'>
 ...
 <li class="menu-item">
  ...
  <?php endif; ?>
    ...
    <img class="d-block rounded"
                          height="100"
                          width="100"
                          id="uploadedAvatar" src="<?php echo $fotoPerfil; ?>" alt="Foto de Perfil">
  <?php else : ?>
    <img class="d-block rounded"
                          height="100"
                          width="100"
                      ...
                      <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                        <div class="row">
                          <div class="mb-3 col-md-6">
                            <label for="firstName" class="form-label">Nome do estabelecimento</label>
                            <input
                              class="form-control"
                              type="text"
                              id="nome_estabelecimento" name="nome_estabelecimento"
                              value="<?php echo $nome_estabelecimento; ?>"
                              autofocus
                            />
                          </div>
                          <div class="mb-3 col-md-6">
                            <label for="lastName" class="form-label">CNPJ</label>
                            <input disabled class="form-control" type="text" name="lastName" id="lastName" value="<?php echo $cnpj; ?>" />
                          </div>
                          <div class="mb-3 col-md-6">
                            <label for="email" class="form-label">Endereço do estabelecimento</label>
                            <input
                              class="form-control"
                              type="text"
                              id="email"

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($resultEntregador->num_rows > 0) {
    ...
    if ($resultVerificarEntregador->num_rows > 0) {
      ...
      // Se a entrega já tiver um entregador diferente, exibir uma mensagem de erro
      if (!empty($entregadorAtual) && $entregadorAtual != $entregadorId) {
        echo "<style>.titulos {display:none}</style><h4 class='mb-2'>Poxa! 😢</h4>Esta entrega já possui outro entregador.";
        exit();
      }
    }

    // Associar o entregador à entrega correspondente
    $sqlAtualizarEntrega = "UPDATE entregas SET entregador = '$entregadorId', status = 'em_rota' WHERE id = '$entregaId'";
      ...
      if ($result->num_rows > 0) {
        ...
        echo "<div class='demo-inline-spacing mt-3'>
          <div class='list-group'>
            <a href='javascript:void(0);' class='list-group-item list-group-item-action active'><strong>Taxa de Entrega:</strong><br> <h4><strong>R$ " . $taxaEntregaFormatada . "</strong></h4></a>

...
<li class="menu-header small text-uppercase">
 ...
 <li class="menu-item">
  ...
  <?php endif; ?>
        ...
        justify-content: space-between;"><h5 class="card-header">Sua localização</h5><div class="card-header"><strong><?php echo $endereco; ?></strong></div></div>
                <div class="card-body">
        ...
        <input type="text" id="start" name="start">
               ...
               <div class="card mb-4">
                <h5 class="card-header">Localização Cliente</h5>
                <div class="card-body">
<div style="padding: 30px;" class="row gx-3 gy-2 align-items-center">
                    <div class="col-md-3">
                      <label class="form-label" for="bairro">Bairro:</label>
                     <select class="form-select placement-dropdown" id="bairro">
            <option value="">Selecione o bairro</option>
            <option value="Jardim Roriz">Jardim Roriz</option>
            <option value="Vila Buritis">Vila Buritis</option>
            <option value="Vila Buritis II">Vila Buritis II</option>
            <option value="Vila Buritis III">Vila Buritis III</option>
            <option value="Vila Buritis IV">Vila Buritis IV</option>
            <option value="Vila de Fátima">Vila de Fátima</option>
            <!-- Adicione aqui as outras opções de bairro -->
        </select>
                    </div>
                    <div class="col-md-3">
                      <label class="form-label" for="conjunto">Conjunto/Modulo:</label>
                      <select class="form-select placement-dropdown" id="conjunto">
            <option value="">Selecione o conjunto/modulo</option>
        </select>
                    </div>
                    <div class="col-md-3">
                      <label class="form-label" for="showToastPlacement">&nbsp;</label>
                      <button id="calculate" class="btn btn-primary d-block"><i class="bx bx-package me-2"></i>Calcular entrega</button>
                    </div>
                  </div>
                          ...
                          <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                                <div class="list-group"><style>#dd:hover, #ddd:hover { color:white}</style>
                          <a id="ddd" class="list-group-item list-group-item-action active"
                            ><span id="dd">Custo de entrega:</span><br><strong><h5 id="taxaDiv"></h5></strong></a
                          >
                          <a class="list-group-item list-group-item-action"
                            >Bairro:<br><div id="bairroModal"></div></a
                          ><br><center><label><strong>Para solicitar uma entrega preencha o endereço do cliente</strong></label></center><input type="text" placeholder="Insira o endereço do cliente" class="list-group-item list-group-item-action form-control centered-input" id="endereco_cliente" name="endereco_cliente" data-restaurante-id="<?php echo $restauranteId; ?>">

                            <center><label><strong>Opcional informar um identificador</strong></label></center>
                          <input type="text" placeholder="Caso precise insira algum identificador para entrega" class="list-group-item list-group-item-action form-control centered-input" id="cod_entrega" name="cod_entrega" data-restaurante-id="<?php echo $restauranteId; ?>">
                              ...
                              <div class="modal-footer d-flex justify-content-center">
                                 ...
                                 background: #ffffff;
                               color:#ea1d2c}</style>
                          <button disabled id="rer" class="btn btn-outline-primary me-1"
                          data-bs-toggle="collapse"
                          href="#collapseExample"
                          role="button"
                          aria-expanded="false"
                          aria-controls="collapseExample"
                        ><i class="bx bx-package me-2"></i>Solicitar Entrega</button>
                        <br>
                        <div class="collapse" id="collapseExample">
                        <div class="d-grid d-sm-flex p-3 border">
                          
                          <span>
                            <center><h4>Os dados da entrega estão corretos?</h4>Confira os dados antes de solicitar uma entrega.</center>
                            <br><center><button data-bs-target="#modalToggle2"
                                  data-bs-toggle="modal"
                                  data-bs-dismiss="modal" type="button" id="solicitarEntrega" class="btn btn-primary"><i class="bx bx-package me-2"></i> Confirmar Entrega</button></center>
                          </span>
                        </div>
                      </div>
...
<div
                        ...
                        >
                          <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="modalToggleLabel2">Entrega Solicitada</h5>
                                <button
                                id="fechar_entregas"
                                  type="button"
                                  class="btn-close"
                              ...
                              <div class="text-center modal-body">
                                 <center><img style="display: none;" id="imagemPerfil" class="w-px-50 rounded-circle" src=""><br></center>

                                 <div id="group_entregador" style="display: none;" class='list-group'>
                              <a class='list-group-item list-group-item-action active'><div id="nomeEntregador"></div></a>
                                  <a class='list-group-item list-group-item-action'><div id="ranking"></div></a>
                                  <a class='list-group-item list-group-item-action'><div id="numEntregas"></div></a>
                                  <a class='list-group-item list-group-item-action'><div id="placaMoto"></div></a>
                                  
                                  <a class='list-group-item list-group-item-action'><div id="horaChegada"></div></a>
                                  Não precisa repassar a taxa de entrega para o entregador.
                                  </div>
                                    
                                <div id="encontrar">Aguarde estamos encontrando um  <strong> entregador</strong>.<br>Não feche essa janela enquanto isso.</div></div>
                              <div class="modal-footer d-flex justify-content-center">
                                <div id="encontrar_loading"><div class="spinner-grow text-danger" role="status">
...
// Função para verificar o estado do input e habilitar/desabilitar o botão
function verificarEstadoInput() {
  var enderecoCliente = $('#endereco_cliente').val();
  var botaoSolicitarEntrega = $('#rer');

  if (enderecoCliente === '') {
    botaoSolicitarEntrega.prop('disabled', true); // Desabilitar o botão
    $('#collapseExample').collapse('hide');
  } else {
    botaoSolicitarEntrega.prop('disabled', false); // Habilitar o botão
  }
}

$('#endereco_cliente').on('input', verificarEstadoInput);

$('#rer').on('click', function(e) {
  var enderecoCliente = $('#endereco_cliente').val();

  if (enderecoCliente === '') {
    alert("Não é possível solicitar entrega com o campo vazio.");
    e.preventDefault(); // Impedir a ação padrão do clique
    return false;
  }
});

// Verificar o estado inicial do input
verificarEstadoInput();
  ...
  $('#calculate').on('click', function() {
    ...
    if (startCoords.length === 2 && endCoords.length === 2) {
      ...
      var bairroModal = document.getElementById('bairroModal');

      taxaDiv.innerHTML = "R$ " + deliveryCost.toFixed(2).replace('.', ',');
      bairroModal.innerHTML = bairro; 
      var bairro = $('#bairro').val(); 
  ...
  $('#solicitarEntrega').on('click', function() {
    ...
    $.ajax({
      ...
      success: function(response) {
        var entregaId = response; // Captura o ID da entrega retornado

        // Chame a função para iniciar a verificação do status da entrega
        verificarStatusEntrega(entregaId);
  ...
  function verificarStatusEntrega(entregaId) {
  var encontrouMotoboy = false; // Variável para controlar se um motoboy foi encontrado

  var interval = setInterval(function() {
    if (encontrouMotoboy) {
      clearInterval(interval); // Interrompe as buscas
      return;
    }

    $.ajax({
      type: 'POST',
      url: 'verificar_status_entrega.php', // Arquivo PHP para verificar o status
      data: { entregaId: entregaId }, // Envia o ID da entrega para verificar o status correto
      success: function(response) {
        // Verifique o valor retornado em 'response' para determinar o status
        if (response === 'em_rota') {
          // O status mudou para 'em_rota', busque o entregador e exiba as informações
          $.ajax({
            type: 'GET',
            url: 'obter_dados_entregador.php', // Arquivo PHP para obter dados do entregador
            data: { entregaId: entregaId }, // Envia o ID da entrega para obter os dados corretos
            dataType: 'json', // Indica que a resposta deve ser interpretada como JSON
            success: function(response) {
              if (response.error) {
                console.error(response.error);
              } else {
                var nome = response.nome;
                var imagemPerfil = response.imagem_perfil;
                var placaMoto = response.placaMoto;
                var horaChegada = response.horaFormatada;
                var numEntregas = response.numEntregas;
                var ranking = response.ranking;
                
                 // Exibe o nome do entregador
                $('#nomeEntregador').text("Entregador: " + nome);
                $('#ranking').text("Ranking: " + ranking);
                $('#numEntregas').text("N° Entregas: " + numEntregas);
                $('#placaMoto').text("Placa moto: " + placaMoto);
                $('#horaChegada').text("Chegar até: " + horaChegada);
                $('#modalToggleLabel2').text("Entregador encontrado");
                $('#group_entregador').css('display', 'block');
                // Define a imagem de perfil do entregador
                $('#imagemPerfil').attr('src', 'http://localhost/entregadores/'+ imagemPerfil);
                $('#imagemPerfil').css('display', 'block');
                $('#encontrar').css('display', 'none');
                $('#encontrar_loading').css('display', 'none');


                encontrouMotoboy = true; // Define a variável como true para interromper as buscas
              }
            },
            error: function(xhr, status, error) {
              console.error(error);
  ...
  }, 5000); // Verificar a cada 5 segundos (5000 milissegundos)
}
 ...
 $('#fechar_entregas').on('click', function() {
  $('#group_entregador').css('display', 'none');
                $('#imagemPerfil').css('display', 'none');
                $('#nomeEntregador').empty();
                $('#encontrar').css('display', 'block');
                $('#encontrar_loading').css('display', 'block');
                $('#modalToggleLabel2').text("Entrega Solicitada");

});


 $('#close_entrega').on('click', function() {
  $('#collapseExample').collapse('hide');
});
    ...
    var data = {
        ...
        "Vila Buritis": [
            ...
            { name: "Quadra 6", coordinates: "-15.629112223451175, -47.6479196548462" },
             { name: "test", coordinates: "-15.625326, -47.722067" },
            // Adicione mais conjuntos conforme necessário
        ],
        "Vila Buritis II": [

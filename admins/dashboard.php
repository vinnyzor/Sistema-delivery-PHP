...
if ($result->num_rows === 1) {
  ...
  $nome_admin_maiusculo = mb_strtoupper($nome_admin, 'UTF-8');
  $fotoPerfil = $row['foto_perfil'];
...
$sqlEntregasHoje = "SELECT COUNT(*) AS total_entregas_hoje FROM entregas WHERE DATE(data_hora) = CURDATE()";
...
$sqlEntregasAnteriores = "SELECT COUNT(*) AS total_entregas_anteriores FROM entregas WHERE DATE(data_hora) < CURDATE()";
...
$sqlEntregas = "SELECT e.*, r.nome_estabelecimento AS nome_restaurante 
               FROM entregas e
               INNER JOIN restaurantes r ON e.restaurante_id = r.id
               WHERE DATE(e.data_hora) = CURDATE()";
...
>
  <head>
    ...
    <title>Dashboard - Fast Delivery</title>
...
<script src="https://cdn.jsdelivr.net/npm/apexcharts@latest"></script>
            ...
            <li class="menu-header small text-uppercase">
             ...
             <li  class="menu-item">
              <a href="entregas.php" class="menu-link">
                ...
                <div data-i18n="Analytics">Todas Entregas</div>
              </a>
            </li>
            ...
            <li class="menu-item">
             ...
             <li class="menu-item">
              <a href="tickets.php" class="menu-link">
                ...
                <div data-i18n="Analytics">Restaurantes</div>
              </a>
            </li>
             <li class="menu-item">
              <a href="tickets.php" class="menu-link">
                <i class="menu-icon tf-icons bx bx-message-rounded-add"></i>
                <div data-i18n="Analytics">Entregadores</div>
              </a>
            </li>

             <li class="menu-header small text-uppercase">
              <span class="menu-header-text">Chamados</span>
            </li>

             <li class="menu-item">
              <a href="chamados.php" class="menu-link">
                <i class="menu-icon tf-icons bx bx-message-dots"></i>
                <div data-i18n="Analytics">Restaurantes <span id="mensagensNaoLidas"></span></div>
              </a>
            </li>
            ...
            <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
              ...
              <div class="navbar-nav align-items-center">
                <div class="nav-item d-flex align-items-center">
                  <div><strong>ADMINISTRADOR - <?php echo $nome_admin_maiusculo; ?></strong></div>
                </div>
              </div>
              <!-- /Search -->
  ...
  <?php endif; ?>
                          ...
                          <div class="flex-grow-1">
                            <span class="fw-semibold d-block"><?php echo $nome_admin_maiusculo; ?></span>
                ...
                <div class="col-lg-8 mb-4 order-0">
                  <div class="card">
                    <div class="d-flex align-items-end row">
                      <div class="col-sm-7">
                        <div class="card-body">
                          <h5 class="card-title text-primary">Bem vindo, Admin <?php echo $nome_admin; ?> 🎉</h5>
                          <p class="mb-4">
                            Restaurantes solicitaram <span class="fw-bold"><?php echo number_format($porcentagemEntregasHoje, 0); ?>%</span> mais entregas hoje. Confira todas as entregas.
                          </p>
...
$sql = "SELECT COUNT(*) AS total_entregas, SUM(taxa_entrega - taxa_entrega_entregador) AS total_taxa_entrega, SUM(CASE WHEN status = 'aguardando_entregador' THEN 1 ELSE 0 END) AS total_em_andamento, SUM(CASE WHEN status = 'em_rota' THEN 1 ELSE 0 END) AS total_em_rota FROM entregas WHERE DATE(data_hora) = '$date'";
                  ...
                  <div class="row">
                    <div class="col-lg-6 col-md-12 col-6 mb-4">
                      <div class="card">
                        ...
                        <?php echo ($totalEntregas > 0) ? ($totalEntregas == 1 ? "1 entrega" : $totalEntregas . " entregas") : "nenhuma"; ?></h5>
                    ...
                    <div class="col-lg-6 col-md-12 col-6 mb-4">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <h5 class="card-title text-nowrap mb-1"><?php echo ($totalEmAndamento > 0) ? ($totalEmAndamento == 1 ? "1 entrega" : $totalEmAndamento . " entregas") : "nenhuma"; ?></h5>
                    ...
                    <div class="col-6 mb-4">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <span class="fw-semibold d-block mb-1">Lucros hoje</span>
                          <h5 style="color: #4acc20 !important" class="card-title text-nowrap mb-2">+R$ 
                            <?php echo number_format($totalTaxaEntrega, 2, ",", "."); ?></h5>
                    ...
                    <div class="col-6 mb-4">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <h5 class="card-title mb-2"><?php echo ($totalEmRota > 0) ? ($totalEmRota == 1 ? "1 entrega" : $totalEmRota . " entregas") : "nenhuma"; ?></h5>
...
$sql = "SELECT DATE_FORMAT(data_hora, '%Y-%m') AS mes, COUNT(*) AS total_entregas FROM entregas WHERE data_hora >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 6 MONTH), '%Y-%m-01') GROUP BY mes";
$result = $conn->query($sql);
...
if ($resultEntregas->num_rows > 0) {
  while ($rowEntrega = $resultEntregas->fetch_assoc()) {
    ...
    <tr>
      <td><?php
        $cod_entrega = $rowEntrega['cod_entrega'];
      ...
      ?></td>
      <td>
  <?php if ($rowEntrega['status'] === 'concluida') { ?>
    <span class="badge badge-center rounded-pill bg-label-success"><i class="bx bx-check"></i></span>
  <?php } elseif ($rowEntrega['status'] === 'aguardando_entregador') { ?>
    <span class="badge badge-center rounded-pill bg-label-warning"><i class="bx bx-loader"></i></span>
  <?php } else { ?>
    <div class="dropdown">
      <button
        type="button"
      ...
      <div class="dropdown-menu">
        <a class="dropdown-item" href="javascript:void(0);" onclick="finalizarEntrega(<?php echo $rowEntrega['id']; ?>)"
          ><i class="bx bx-check me-1"></i> Finalizar entrega</a
        >
        <?php if ($rowEntrega['status'] === 'em_rota') { ?>
          <a class="dropdown-item" href="javascript:void(0);" onclick="exibirDetalhesEntregador(<?php echo $rowEntrega['entregador']; ?>)"
            ><i class="bx bx-street-view me-1"></i> Entregador</a
          >
        <?php } ?>
      </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalEntregadorLabel">Detalhes do Entregador</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <center><img id="imagemPerfil" class="w-px-50 rounded-circle" src=""></center><br>

         <div id="group_entregador" class='list-group'>
                              <a class='list-group-item list-group-item-action active'><strong>Nome:</strong> <span id="nomeEntregador"></span></a>
                                  <a class='list-group-item list-group-item-action'><strong>Ranking:</strong> <span id="ranking"></span>°</a>
                                  <a class='list-group-item list-group-item-action'><strong>N° Entregas:</strong> <span id="num_entregas"></span></a>
                                  <a class='list-group-item list-group-item-action'><strong>Placa Moto:</strong> <span id="placaMoto"></span></a>
                                  
                                  <a class='list-group-item list-group-item-action'><strong>Telefone:</strong> <span id="telefone"></span></a>
                                  <center><strong>Não precisa repassar a taxa de entrega para o entregador.</strong></center>
                                  </div>
        <!-- Adicione outros campos de acordo com a estrutura da tabela entregadores -->
      </div>

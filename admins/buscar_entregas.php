...
if ($result->num_rows === 1) {
  ...
  if ($periodo === 'diaAtual') {
    $sql = "SELECT * FROM entregas WHERE restaurante_id='$restauranteId' AND DATE(data_hora) = CURDATE()";
  } else if ($periodo === 'semanaAtual') {
    $sql = "SELECT * FROM entregas WHERE restaurante_id='$restauranteId' AND WEEK(data_hora) = WEEK(CURDATE())";
  } else if ($periodo === 'mesAtual') {
    $sql = "SELECT * FROM entregas WHERE restaurante_id='$restauranteId' AND MONTH(data_hora) = MONTH(CURDATE())";
